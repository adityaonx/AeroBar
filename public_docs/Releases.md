...
- **Auto-Updater Reliability**: Fixed a major bug where the auto-updater could strand a broken backup and fail to swap the app bundle (resulting in an "AeroBar not found" error) due to extended-attribute copy failures from the downloaded DMG. The updater now uses a completely detached background script to perform the update. Even if AeroBar suffers a hard crash while attempting to install the update, the independent script will safely complete the DMG mounting, bundle swap, and relaunch sequence in the background, making the update process 100% crash-proof.
